...
static void cria_vertices_insercao(Grafo g, int origem, int destino){
    ...
    if(!destino_existe && origem !=destino) g_vertice_cria(g, destino);
}

static void g_ins_aresta_inversa(Grafo g, int origem, int destino, double peso){
    ...
    while(no_lista_atual !=NULL){
        ...
        if(lista_atual->elemento_de_partida == origem){
            lista_insere(lista_atual, no_nova_aresta, -1);
            break;
        }
        no_lista_atual = no_lista_atual->prox;
    }
...
void g_ins_aresta(Grafo g, int origem, int destino, double peso) {
    assert(g!=NULL);
    ...
    bool achei = false;
    while(no_lista_atual !=NULL && achei==false){
        lista* lista_atual = no_lista_atual->data;
        if(lista_atual->elemento_de_partida == origem){
            lista_insere(lista_atual, no_nova_aresta, -1);
            achei =true;
    ...
    if(!g->orientado && origem!=destino) g_ins_aresta_inversa (g, destino, origem, peso);
    g->num_arestas++; 
}
...
void g_rem_aresta(Grafo g, int origem, int destino){
    ...
    if(!g->orientado && origem != destino){
        g_rem_aresta_inversa(g, destino, origem);
    }
    g->num_arestas--;
}
...
void grafo_testa() {
    ...
    g_ins_aresta(g, 1, 2, 1.0);
    g_rem_aresta(g, 1, 2);

    g_ins_aresta(g, 2, 2, 0.0);
    g_rem_aresta(g, 2, 2);
    
    g_ins_aresta(g, 1, 3, 2.0);
    g_ins_aresta(g, 2, 3, 3.0);
    g_ins_aresta(g, 3, 4, 4.0);
    g_ins_aresta(g, 4, 1, 5.0);
